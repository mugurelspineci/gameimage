...
//						fileName = fixPath(fileName);
						...
						if(MSIE){
                            ...
                            cell.innerHTML = '<a href="http:'+divisionNameFile[i]+'">11i' + dayValue + '</a>';

						} 
						else{ // Otherwise
                             cell.innerHTML = '<a href="http:'+divisionNameFile[i]+'">11e' + dayValue + '</a>';
					...
					//HuynhTran -->add
						...
						for(j = 0; j< numFile; j++){						
							i++;
							fileName = "'" + divisionNameFile[i] + "'";  
							fileName = fixPath(fileName);
							filenameT = fileName.replace(/^.*\\/, '')
							html_p+='<p class="rowp"><a href="http:'+fileName+'">'+ filenameT +'</a></p>';
						}
						
						linka=document.createElement('a');
						linka.appendChild(document.createTextNode("11e" +dayValue));
						linka.href="javascript:void(0);";
						linka.onclick=function(){
								//left = (screen.width/2)-(popWidth/2);
//								top = (screen.height/2)-(popHeight/2);						
								//newwindow2=window.open('','name','toolbar=no, location=no, directories=no, status=no, menubar=no, scrollbars=yes, resizable=no, copyhistory=no, width=' + popWidth + ', height=' + popHeight + ', top=' + top + ', left=' + left);
	...
	for(var no=1;no<=daysInMonth;no++){
		...
		if(tempYearHaveData){
			...
			for(var i=idxstart; i<=idxend; i++) {
				//HuynhTran -->edit 
				//var tt = divisionName[i] +" == "+ divisionNameFile[i].length
				//document.getElementById('iptemp').innerHTML = tt;
				//if(fullDate == divisionName[i] && divisionNameFile[i].length>1){
				if(fullDate == divisionName[i] && divisionNameFile[i].length>3){
					// Detect IE browser
					if(MSIE){
                        cell.innerHTML = '<a href="http:'+divisionNameFile[i]+'">13i' + no + '</a>';						
				
					} 
					else{ // Otherwise
                        cell.innerHTML = '<a href="http:'+divisionNameFile[i]+'">13eo' + no + '</a>';
					}
					//document.getElementById('iptemp').innerHTML = tt;
					isHaveData = true;
					break;
				...
				//HuynhTran -->add
					...
					for(j = 0; j< numFile; j++){						
						i++;
       					filenameT = divisionNameFile[i].replace(/^.*\\/, '')
						html_p+='<p class="rowp"><a href="http:'+divisionNameFile[i]+'">'+ filenameT +'</a></p>';
					}					
					linka=document.createElement('a');
					linka.appendChild(document.createTextNode("13e" + no));
					linka.href="javascript:void(0);";
					linka.onclick=function(){
							//left = (screen.width/2)-(popWidth/2);
//							top = (screen.height/2)-(popHeight/2);						
							//newwindow2=window.open('','name','toolbar=no, location=no, directories=no, status=no, menubar=no, scrollbars=yes, resizable=yes, copyhistory=no, width=' + popWidth + ', height=' + popHeight + ', top=' + top + ', left=' + left);
							newwindow2=window.open('','PMeye',confPop);
							tmp = newwindow2.document;
							tmp.write('<html><head><title>PMeye Ver. 2.0</title>');
							tmp.write('<style>');
			...
			if(tempYearHaveData){
				for(var i=idxstart; i<=idxend; i++) {
					...
					//alert(datefull);
						...
						// Detect IE browser
						if(MSIE){
                            cell.innerHTML = '<a href="http:'+divisionNameFile[i]+'">14i' + (no + 1) + '</a>';
						} 
						else{ // Otherwise

                            cell.innerHTML = '<a href="http:'+divisionNameFile[i]+'">14e' + (no + 1) + '</a>';
						}
						isHaveData = true;
						break;
					}
					//HuynhTran -->add
						...
						linka=document.createElement('a');
						linka.appendChild(document.createTextNode("14e" + dayValue));
						linka.href="javascript:void(0);";
						linka.onclick=function(){
								//left = (screen.width/2)-(popWidth/2);
//								top = (screen.height/2)-(popHeight/2);						
